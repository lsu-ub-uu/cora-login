...
public final class AuthTokenToJsonConverterImp implements AuthTokenToJsonConverter {
	...
	private void possiblyAddFirstNameToJson(JsonArrayBuilder userChildren) {
		Optional<String> oFirstName = authToken.firstName();
		if (oFirstName.isPresent()) {
			JsonObjectBuilder firstName = createObjectBuilderWithName("firstName");
			firstName.addKeyString(VALUE, String.valueOf(oFirstName.get()));
			userChildren.addJsonObjectBuilder(firstName);
		}
	}

	private void possiblyAddLastNameToJson(JsonArrayBuilder userChildren) {
		Optional<String> oLastName = authToken.lastName();
		if (oLastName.isPresent()) {
			JsonObjectBuilder lastName = createObjectBuilderWithName("lastName");
			lastName.addKeyString(VALUE, String.valueOf(oLastName.get()));
